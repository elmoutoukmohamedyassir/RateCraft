...
const RELATED_POSTS = [
  { id: 'quote-a-project-profitably',         label: 'The pre-send checklist for project quotes'    },
  { id: 'how-to-price-a-freelance-project',   label: 'Full method for pricing any freelance project' },
  { id: 'freelance-project-pricing-template', label: 'What to include in every project quote'        },
  { id: 'hourly-vs-project-pricing',          label: 'When to use project pricing vs hourly'         },
];

const MISTAKES = [
  { n: '01', title: 'Revision rounds not priced', desc: 'Revision rounds are real work. "Two rounds included" without pricing them explicitly means you absorb the cost. The calculator prices them separately.' },
  { n: '02', title: 'No scope buffer',             desc: 'Every project takes longer than the estimate. A 15–25% buffer is not pessimism — it is professional risk management.' },
  { n: '03', title: 'Tax not included in the quote', desc: 'Quoting before tax and netting after is how projects feel profitable until year-end. Build tax into every number you send.' },
  { n: '04', title: 'No profit margin',            desc: 'A project that covers costs builds nothing. Margin — 10–20% on the buffered subtotal — is what makes the project worthwhile beyond immediate income.' },
  { n: '05', title: 'Hourly floor is wrong',       desc: 'Every error in the hourly rate multiplies through the project price. Fix the floor first, then build the quote.' },
];
...
export default function ProjectCalculatorPage() {
  ...
  return (
    <div style={{ background: 'var(--color-ink-950)', minHeight: '100vh' }}>
      <SEO
        title="Project Pricing Calculator for Freelancers | RateCrafts"
        description="Price any freelance project correctly. Enter your hourly rate, estimated hours, revision rounds, buffer, and margin — get a complete project quote with the math done."
      />
      ...
      <header style={{ padding: '5rem 1.5rem 3.5rem', borderBottom: '1px solid var(--color-ink-800)', position: 'relative', overflow: 'hidden' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} style={{ maxWidth: '640px', position: 'relative', zIndex: 1 }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 900, color: 'var(--color-ink-50)', letterSpacing: '-0.03em', lineHeight: 1.05, marginBottom: '1rem' }}>
            ...
            <br />
            <em style={{ color: 'var(--color-brass-300)', fontStyle: 'italic' }}>that holds up.</em>
          </h1>
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: '1rem', fontWeight: 300, color: 'var(--color-ink-300)', lineHeight: 1.75, maxWidth: '480px', marginBottom: '1.5rem' }}>
            Enter your hourly rate, project scope, revision rounds, buffer, and margin.
            Get a final price that covers everything — including what usually gets forgotten.
          </p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }} className="lg:col-span-7">
          ...
          <InputSection label="01 — Your Rate &amp; Scope">
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
              <ProjField label="Your Hourly Rate" name="hourlyRate" value={state.hourlyRate} onChange={handleChange} prefix="$"
                tooltip="Your minimum viable hourly rate. If you have not calculated this yet, use the hourly rate calculator first — this is the most important input in the entire form." />
              <ProjField label="Estimated Project Hours" name="estimatedHours" value={state.estimatedHours} onChange={handleChange} suffix="hrs"
                tooltip="Total hours across all phases: discovery, design/development, testing, handoff. Estimate by phase, not as a total — phase estimates are more accurate." />
            </div>
          </InputSection>

          <InputSection label="02 — Revision Rounds">
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
              <ProjField label="Number of Revision Rounds" name="revisionRounds" value={state.revisionRounds} onChange={handleChange} suffix="rounds"
                tooltip="How many rounds of consolidated feedback are included. One round = one set of feedback, applied once. Be specific in your quote about what counts as a round." />
              <ProjField label="Hours per Revision Round" name="hoursPerRevision" value={state.hoursPerRevision} onChange={handleChange} suffix="hrs"
                tooltip="How long a revision round typically takes: the review call, the changes, any back-and-forth. Most rounds take 2–5 hours depending on project complexity." />
            </div>
            <div style={{ marginTop: '1.25rem', padding: '1rem 1.25rem', background: 'var(--color-ink-800)', border: '1px solid var(--color-ink-700)' }}>
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.62rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--color-ink-500)', marginBottom: '0.25rem' }}>Total revision provision</p>
              <p style={{ fontFamily: 'var(--font-display)', fontSize: '1.3rem', fontWeight: 700, color: 'var(--color-ink-100)', letterSpacing: '-0.02em' }}>
                ...
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.72rem', color: 'var(--color-ink-500)', fontWeight: 400, marginLeft: '0.5rem' }}>
          ...
          <InputSection label="03 — Buffer, Tax &amp; Margin">
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
              <RangeField label="Project Buffer" name="bufferPercentage" value={state.bufferPercentage} onChange={handleChange} suffix="%" max={50}
                tooltip="Added to the base + revision total to absorb scope creep and estimation errors. 15–25% is professional practice. This is not padding — it is risk management." />
              <RangeField label="Tax Percentage" name="taxPercentage" value={state.taxPercentage} onChange={handleChange} suffix="%" max={60}
                tooltip="Your effective combined tax rate. This is applied to the buffered subtotal so the final price reflects what you need to charge — not what you net." />
              <RangeField label="Profit Margin" name="profitMarginPercentage" value={state.profitMarginPercentage} onChange={handleChange} suffix="%" max={50}
                tooltip="Applied on top of costs. Without margin, the project covers expenses but builds nothing. 10–20% is standard for most professional freelance engagements." />
            </div>
          </InputSection>

          <InputSection label="Why This Matters">
            <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.9rem', fontWeight: 300, color: 'var(--color-ink-300)', lineHeight: 1.8, marginBottom: '1.5rem' }}>
              Project pricing fails in predictable ways. This calculator builds in the four things most freelancers forget: revision rounds, scope buffer, tax, and profit margin. What you see in the results panel is the price to send — not the price before everything else gets subtracted.
            </p>
            ...
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.8rem', background: 'var(--color-ink-800)', border: '1px solid var(--color-ink-700)', color: 'var(--color-ink-200)', padding: '1rem 1.25rem', lineHeight: 1.9, marginBottom: '2rem' }}>
              ...
              <span style={{ color: 'var(--color-brass-400)' }}>buffered subtotal + tax + profit = what to charge</span>
            </div>

            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '1.1rem', fontWeight: 700, color: 'var(--color-ink-50)', letterSpacing: '-0.015em', marginBottom: '1.25rem' }}>
              5 reasons project quotes go wrong
            </h2>
            ...
            <div style={{ padding: '1.5rem', background: 'var(--color-ink-900)', border: '1px solid var(--color-ink-800)', borderLeft: '2px solid var(--color-brass-500)', marginBottom: '2rem' }}>
              <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.875rem', fontWeight: 300, color: 'var(--color-ink-300)', lineHeight: 1.7, marginBottom: '0.85rem' }}>
                Every error in your hourly rate multiplies through every project quote. If you have not verified your rate against your real costs, do that first.
              </p>
              <Link to="/calculator" style={{ fontFamily: 'var(--font-mono)', fontSize: '0.68rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--color-brass-300)', textDecoration: 'none' }}>
                → Calculate your hourly floor first
              </Link>
            </div>
...
function ProjectResultsPanel({ results, state }: { results: ProjectResults; state: ProjectState }) {
  ...
  const rows = [
    ...
    { label: 'Revision provision',                        value: fmt(results.revisionCost),       note: `${state.revisionRounds} rounds × ${state.hoursPerRevision} hrs` },
    { label: `Scope buffer (${state.bufferPercentage}%)`, value: fmt(results.bufferAmount),       note: 'Risk management' },
    { label: `Tax (${state.taxPercentage}%)`,             value: fmt(results.taxAmount),          note: 'On buffered subtotal' },
    { label: `Profit (${state.profitMarginPercentage}%)`, value: fmt(results.profitAmount),       note: 'Business margin' },
  ];

  const handleEmailSubmit = (e: React.FormEvent) => { e.preventDefault(); if (email.trim()) setSubmitted(true); };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} style={{ background: 'var(--color-ink-900)', border: '1px solid var(--color-ink-800)', padding: '2rem', position: 'relative', overflow: 'hidden' }}>
        ...
        <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: 'var(--color-ink-500)' }}>Total to quote the client</p>
      </motion.div>
      ...
      <div style={{ background: 'var(--color-ink-900)', border: '1px solid var(--color-ink-800)', borderLeft: '2px solid var(--color-brass-600)', padding: '1.5rem' }}>
        {!submitted ? (
          <>
            ...
            <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.82rem', fontWeight: 300, color: 'var(--color-ink-300)', lineHeight: 1.6, marginBottom: '1rem' }}>Want a PDF summary of this project quote? Enter your email.</p>
            <form onSubmit={handleEmailSubmit} style={{ display: 'flex', flexDirection: 'column' as const }}>
          ...
          <div style={{ textAlign: 'center', padding: '0.5rem 0' }}>
            <p style={{ fontFamily: 'var(--font-display)', fontSize: '1rem', fontWeight: 700, color: 'var(--color-brass-300)', marginBottom: '0.3rem' }}>✓ On the list</p>
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: 'var(--color-ink-600)' }}>Project quote breakdown coming to {email}</p>
          </div>
        )}
      </div>

      <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.6rem', color: 'var(--color-ink-600)', lineHeight: 1.6, letterSpacing: '0.03em', padding: '0.5rem 0' }}>Estimates only. Consult a financial advisor for business planning.</p>
    </div>
  );
}
      ...
      <label style={{ fontFamily: 'var(--font-mono)', fontSize: '0.66rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--color-ink-400)', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
        ...
        {tooltip && (
          <span style={{ position: 'relative', display: 'inline-flex' }} onMouseEnter={() => setShowTip(true)} onMouseLeave={() => setShowTip(false)}>
            ...
            {showTip && <div style={{ position: 'absolute', bottom: 'calc(100% + 8px)', left: '50%', transform: 'translateX(-50%)', width: '230px', padding: '0.65rem 0.85rem', background: 'var(--color-ink-800)', border: '1px solid var(--color-ink-700)', color: 'var(--color-ink-200)', fontFamily: 'var(--font-sans)', fontSize: '0.78rem', fontWeight: 300, lineHeight: 1.55, zIndex: 100, pointerEvents: 'none', textTransform: 'none', letterSpacing: 'normal' }}>{tooltip}</div>}
          </span>
        )}
      </label>
